...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .mini-chart{height:96px;display:flex;align-items:flex-end;gap:5px;padding:18px 2px 18px;position:relative;border-bottom:1px solid #edf1f5;margin-top:6px}.mini-trend{position:absolute;inset:8px 0 18px 0;pointer-events:none;z-index:1}.mini-trend polyline{fill:none;stroke:#94a3b8;stroke-width:1.8;stroke-dasharray:4 4;stroke-linecap:round;stroke-linejoin:round;opacity:.55}.mini-bar-wrap{height:100%;flex:1;display:flex;align-items:flex-end;position:relative;z-index:2}.mini-bar{width:100%;border-radius:3px 3px 1px 1px;min-height:2px;opacity:.72;transition:opacity .15s,transform .15s}.mini-bar.current{opacity:1}.mini-bar:hover{opacity:1;transform:translateY(-2px)}.mini-month{position:absolute;left:50%;transform:translateX(-50%);bottom:-16px;font-size:8px;color:#8794a7;white-space:nowrap}.mini-label{position:absolute;left:50%;transform:translateX(-50%);bottom:calc(var(--bar-h) + 4px);font-size:10px;font-weight:900;color:#334159;white-space:nowrap;z-index:3}
...
<body>
  ...
  <script>
    ...
    function getFilteredData(options={}){const ignoreStatus=!!options.ignoreStatus;return rawData.filter(d=>{
    ...
    function renderSavings(summary){const rows=summary.filter(r=>r.savings>0).sort((a,b)=>a.savings-b.savings);if(!rows.length){document.getElementById('savingsChart').innerHTML='<div class="empty-state">No completed savings data in the current selection.</div>';return}const max=Math.max(...rows.map(r=>r.savings));const layout=basePlotLayout({margin:{l:100,r:70,t:5,b:38}});layout.xaxis.tickprefix='$';layout.xaxis.tickformat='~s';layout.xaxis.range=[0,max*1.25];Plotly.react('savingsChart',[{type:'bar',orientation:'h',x:rows.map(r=>r.savings),y:rows.map(r=>shortName(r.supervisor)),marker:{color:rows.map(r=>supervisorColors[r.supervisor])},text:rows.map(r=>r.savings>=1000?`$${Math.round(r.savings/1000)}k`:fmtMoney.format(r.savings)),textposition:'outside',cliponaxis:false,customdata:rows.map(r=>r.supervisor),hovertemplate:'%{customdata}<br>Completed savings: $%{x:,.0f}<extra></extra>'}],layout,{displayModeBar:false,responsive:true})}

    function updateKpis(data){
      const total=data.length,completed=data.filter(d=>d.status==='Completed').length,savings=data.reduce((a,d)=>a+(Number(d.completedSavings)||0),0),m=latestVisibleMonth(data),sups=selectedSupervisors(),monthRows=m?data.filter(d=>submittedMonth(d)===m):[];const activePeople=new Set(data.map(d=>String(d.submitter||d.owner||d.ownerRaw||'').trim()).filter(Boolean));const hc=sups.reduce((a,s)=>a+headcount(s,m),0);const avg=hc?monthRows.length/hc:0,activeRate=hc?100*activePeople.size/hc:0;
